...
url_api = 'http://alexandrejeandel.pythonanywhere.com/'     # online API
...
def main():
    ...
    st.markdown(f"This web application allows you to know the probability for a client to re embourse their loans. \
                 Some of the parameters are {get_label_list_str(label_list)}.", unsafe_allow_html=True)
    ...
    feature_x = st.sidebar.selectbox("Select the first feature", options=list(chart_option_dict.values()), index=0, key='feature_x')
    feature_y = st.sidebar.selectbox("Select the second feature", options=list(chart_option_dict.values()), index=1, key='feature_y')

    if st.sidebar.button("Generate Bivariate Plot"):
        if feature_x and feature_y:
            # Mapping back to the actual column names
            feature_x_col = list(chart_option_dict.keys())[list(chart_option_dict.values()).index(feature_x)]
            feature_y_col = list(chart_option_dict.keys())[list(chart_option_dict.values()).index(feature_y)]
            df_bivariate = load_bivariate_data(feature_x_col, feature_y_col)
            plot_bivariate(df_bivariate, feature_x_col, feature_y_col)
...
# Function to fetch data for two selected features and return a DataFrame
def load_bivariate_data(feature_x, feature_y):
    data_x = load_data(feature_x)
    data_y = load_data(feature_y)
    df = pd.DataFrame({feature_x: data_x, feature_y: data_y})
    return df

# Function to plot bivariate scatter plot
def plot_bivariate(df, feature_x, feature_y):
    fig, ax = plt.subplots()
    ax.scatter(df[feature_x], df[feature_y], alpha=0.5)
    ax.set_xlabel(feature_x)
    ax.set_ylabel(feature_y)
    ax.set_title(f'Bivariate Plot of {feature_x} vs {feature_y}')
    st.pyplot(fig)
